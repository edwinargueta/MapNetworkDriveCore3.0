...
namespace MapCodeNetwork
{
    ...
    public partial class App : Application
    {
        ...
        private static void CopyAllFiles(DirectoryInfo currDir, string CodePath)
        {
            ...
            Parallel.ForEach(LocalFiles, (localFile) =>
            {
                ...
                try
                {
                    ...
                    if (localFile.LastWriteTime >= DateTime.Now.AddDays(-1)
                            || (DateTime.Now.Day == 1 && localFile.LastWriteTime >= DateTime.Now.AddDays(-3)))
                    {
        ...
        private async void Application_Startup(object sender, StartupEventArgs e)
        {
            try
            {
                ...
                worker.DoWork += delegate (object s, DoWorkEventArgs args)
                {
                    ...
                    {
                        String Msg = "All files successfully backed up! Total number of files: " + NumSuccess;
                        notifier.ShowSuccess(Msg);
                    }
                };
                worker.RunWorkerAsync();
                using (FileStream stream = File.Create(Path.Combine("..\\..\\..\\", "TimeLog.txt")))
                {
                    using (StreamWriter writer = new StreamWriter(stream))
                    {
                        writer.WriteLine("Total Amount of time elapsed: {0:hh\\:mm\\:ss}", stopwatch.Elapsed);
                    }
                }
